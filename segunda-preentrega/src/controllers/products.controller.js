...
export const getProducts = async (req,res)=>{
    try {
        ...
        const prods = await productsModel.paginate(filQuery,{limit:limite,page:pag,orderQuery,lean:true})
...
export const createProduct = async (req,res)=>{
    try {
        ...
        const newProduct = await productsModel.create({title,description,category,price,stock,code})
        res.send(newProduct)
    } catch (error) {
        res.status(500).send(error)
    }
...
export const deleteProduct = async (req,res)=>{
    try {
        const pid = req.params.pid
        const updateProd = req.body
        const rta = await productsModel.findByIdAndDelete(pid,updateProd)
        if(rta)
